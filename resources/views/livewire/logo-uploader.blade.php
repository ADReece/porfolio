<div class="space-y-4" x-data>
    ...
    @if(session('logo-status'))
        <div x-data="{ show: true }" x-show="show" x-transition.opacity.duration.300ms
             ...
             class="text-xs px-3 py-2 rounded bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">
            {{ session('logo-status') }}
        </div>
    @endif
    ...
    @else
        <div class="flex items-center gap-4">
            @if($currentLogoUrl)
                <img src="{{ $currentLogoUrl }}" alt="Logo" class="h-16 max-w-[10rem] object-contain rounded shadow" />
            @elseif($logo)
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="flex gap-2">
                    <button wire:click="save" wire:loading.attr="disabled" @disabled="!$logo" class="px-3 py-1 text-xs rounded bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50">Save Logo</button>
                    @if($currentLogoUrl)
                        <button wire:click="remove" wire:loading.attr="disabled" class="px-3 py-1 text-xs rounded bg-red-600 text-white hover:bg-red-700 disabled:opacity-50">Remove</button>
                    @endif
                </div>
